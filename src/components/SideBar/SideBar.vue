...
>
    ...
    <div class="fixed top-0 left-0 z-20 w-64 h-full transition-all duration-500 transform -translate-x-full bg-white shadow-lg peer-checked:translate-x-0">
      <div class="px-6 py-4">
        <h2 class="text-xl font-semibold">Manager Mecânica</h2>
        ...
        <div class="flex space-x-20">
            ...
            <div class="flex-colums mt-5">
              <router-link class="flex items-center gap-x-3.5 py-2 px-2.5 text-lg" :to="{name: 'DashBoard'}" >
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-printer size-5"><polyline points="6 9 6 2 18 2 18 9"></polyline><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"></path><rect x="6" y="14" width="12" height="8"></rect></svg>
                Dashboard
                </router-link> 
                <router-link class="flex items-center gap-x-3.5 py-2 px-2.5 text-lg" :to="{name: 'Vehicles'}" >
                  <svg class="size-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512"><path d="M575.8 255.5c0 18-15 32.1-32 32.1h-32l.7 160.2c.2 35.5-28.5 64.3-64 64.3H128.1c-35.3 0-64-28.7-64-64V287.6H32c-18 0-32-14-32-32.1c0-9 3-17 10-24L266.4 8c7-7 15-8 22-8s15 2 21 7L564.8 231.5c8 7 12 15 11 24zM352 224a64 64 0 1 0 -128 0 64 64 0 1 0 128 0zm-96 96c-44.2 0-80 35.8-80 80c0 8.8 7.2 16 16 16H384c8.8 0 16-7.2 16-16c0-44.2-35.8-80-80-80H256z"/></svg>                    
                  Veículos
                </router-link>                
                <router-link class="flex items-center gap-x-3 py-2 px-2.5 text-lg" :to="{name: 'Services'}" >
                ...
                <router-link class="flex items-center gap-x-3.5 py-2 px-2.5 text-lg" :to="{name: 'Owners'}">
...
export default {
  ...
  methods:{
    ...
    async handleSubmit() {
      try{
        this.loading = true
        let {data} = await http.get('/results')
        data = data.templates
        const foundResult = data.find(obj => obj.slug.toLowerCase() === this.searchQuery.toLowerCase() || obj.name.toLowerCase() === this.searchQuery.toLowerCase());
                this.loading=false;
        !foundResult? alert("Diagnóstico Não Encontrado") : router.push({ name: 'DiagnosticoController', params: { id: foundResult.slug } }
      )
      this.searchQuery = ''
      }catch(error){
        this.loading=false;
    ...
    async adicionarTemplate(){
      ...
      {
        try {
          await http.post('/template',{"nameTemplate":name,"name":auth.user.name});
        } catch (error) {
          console.log(error)
        }        
      }
